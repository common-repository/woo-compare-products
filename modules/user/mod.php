<?php
class userWcp extends moduleWcp {
    ...
    public function isAdmin() {
		if(!function_exists('wp_get_current_user')) {
			frameWcp::_()->loadPlugins();
		}
        return current_user_can( frameWcp::_()->getModule('adminmenu')->getMainCap() );
    }
	...
	protected function _loadUserData() {
		if(!$this->_dataLoaded) {
			if(!function_exists('wp_get_current_user')) frameWcp::_()->loadPlugins();
	...
	public function getAdminsList() {
		global $wpdb;
		$admins = dbWcp::get('SELECT * FROM #__users 
