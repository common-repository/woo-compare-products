<?php
class supsystic_promoModelWcp extends modelWcp {
	private $_apiUrl = '';
	...
	public function saveUsageStat($code, $unique = false) {
		...
		$query = 'INSERT INTO @__usage_stat SET code = "'. dbWcp::escape($code). '", visits = 1
			ON DUPLICATE KEY UPDATE visits = visits + 1';
		return dbWcp::query($query);
	}
	private function _checkUniqueStat($code) {
		$uniqueStats = get_option(WCP_CODE. '_unique_stats');
		...
		$uniqueStats[] = $code;
		update_option(WCP_CODE. '_unique_stats', $uniqueStats);
		return false;
	}
	...
	public function getUserStatsCount() {
		...
		return (int) dbWcp::get($query, 'one');
	}
	...
	public function getTourHst() {
		$hst = get_user_meta(get_current_user_id(), WCP_CODE . '-tour-hst', true);
		if(empty($hst))
			$hst = array();
		if(!isset($hst['passed']))
			$hst['passed'] = array();
	...
	public function setTourHst( $hst ) {
		update_user_meta(get_current_user_id(), WCP_CODE . '-tour-hst', $hst);
	}
	public function clearTourHst() {
		delete_user_meta(get_current_user_id(), WCP_CODE . '-tour-hst');
	}
