...
class errorsWcp {
    ...
    static public function setSession($error) {
        ...
        if(is_array($error))
            ...
            $sesErrors[] = $error;
        reqWcp::setVar('sesErrors', $sesErrors, 'session');
    }
    static public function init() {
        $wcpErrors = reqWcp::getVar('wcpErrors');
        if(!empty($wcpErrors)) {
            if(!is_array($wcpErrors)) {
                $wcpErrors = array( $wcpErrors );
            }
            $wcpErrors = array_map('htmlspecialchars', array_map('stripslashes', array_map('trim', $wcpErrors)));
            if(!empty($wcpErrors)) {
                self::$current = $wcpErrors;
				if(is_admin()) {
					add_action('admin_notices', array('errorsWcp', 'showAdminErrors'));
				} else {
					add_filter('the_content', array('errorsWcp', 'appendErrorsContent'), 99999);
				}
            }
        }
    }
    ...
    static public function getSession() {
        return reqWcp::getVar('sesErrors', 'session');
    }
    static public function clearSession() {
        reqWcp::clearVar('sesErrors', 'session');
    }
