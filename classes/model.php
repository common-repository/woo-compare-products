<?php
abstract class modelWcp extends baseObjectWcp {
	...
	public function getModule() {
		return frameWcp::_()->getModule( $this->_code );
	}
	...
	protected function _buildQuery($table = null) {
		if(!$table)
			$table = frameWcp::_()->getTable( $this->_tbl );
	...
	public function removeGroup($ids) {
		...
		if(!empty($ids)) {
			if(frameWcp::_()->getTable( $this->_tbl )->delete(array('additionalCondition' => 'id IN ('. implode(',', $ids). ')'))) {
				$this->_afterRemove( $ids );
				return true;
			} else 
				$this->pushError (__('Database error detected', WCP_LANG_CODE));
		} else
			$this->pushError(__('Invalid ID', WCP_LANG_CODE));
		return false;
	}
	...
	public function remove($id) {
		$id = (int) $id;
		if($id) {
			if(frameWcp::_()->getTable( $this->_tbl )->delete(array('id' => $id))) {
	...
	public function clear() {
		$res = $this->delete();	// Just delete all
		if($res) {
			$this->_afterRemove();
		}
		return $res;
	...
	public function delete($params = array()) {
		if(frameWcp::_()->getTable( $this->_tbl )->delete( $params )) {
			$this->_afterRemove( $params );
			return true;
		} else 
			$this->pushError (__('Database error detected', WCP_LANG_CODE));
		return false;
	...
	public function getById($id) {
		...
		return $this->getOneBy($this->_idField, $id);
	}
	public function getBy($field, $value, $params = array(), $returnOne = false) {
		$data = $this->setWhere(array($field => $value))->getFromTbl( $params );
		if(!empty($data)) {
			return $returnOne && is_array($data) ? array_shift($data) : $data;
		}
		return false;
	}
	public function getOneBy($field, $value, $params = array()) {
		return $this->getBy($field, $value, $params, true);
	}
	public function insert($data) {
		$data = $this->_dataSave($data, false);
		$id = frameWcp::_()->getTable( $this->_tbl )->insert( $data );
		if($id) {
			return $id;
		}
		$this->pushError(frameWcp::_()->getTable( $this->_tbl )->getErrors());
		return false;
	}
	public function updateById($data, $id = 0) {
		...
		if($id) {
			...
			$this->pushError(__('Empty or invalid ID', WCP_LANG_CODE));
		return false;
	}
	public function update($data, $where) {
		$data = $this->_dataSave($data, true);
		if(frameWcp::_()->getTable( $this->_tbl )->update( $data, $where )) {
			return true;
		}
		$this->pushError(frameWcp::_()->getTable( $this->_tbl )->getErrors());
		return false;
	...
	public function getView($code = '') {
		return frameWcp::_()->getModule( $this->_code )->getController()->getView($code);
	}
	public function translate($str) {
		return __($str, WCP_LANG_CODE);
	}
}
