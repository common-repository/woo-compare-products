...
class validatorWcp {
    ...
    static public function validate($field, $label = '', $validate = array()) {
        self::$errors = array();
        if(is_object($field) && get_class($field) != 'fieldWcp') {
            $value = $field;
            $field = new fieldWcpWcp('noMatter');
            $field->label = $label;
        ...
        if(!empty($field->validate)) {
            foreach($field->validate as $v) {
                 if(method_exists('validatorWcp', $v))
                    self::$v($field);
            }
        }
        if(method_exists('validatorWcp', $field->type)) {
            $validate = $field->type;
            self::$validate($field);
        }
        if($field->maxlen) {
    ...
    static public function getProductValidationMethods() {
        $res = array();
        $all = get_class_methods('validatorWcp');
        foreach($all as $m) {
