...
class langWcp {
	...
	static public function extract($d = array('dir' => '', 'langWcp' => '')) {
		$data = array();
		if(isset($d['dir']) && !empty($d['dir']))
			$langDirPath = $d['dir'];
		else if(isset($d['langWcp']) && !empty($d['langWcp']))
			$langDirPath = WCP_LANG_DIR. $d['langWcp']. DS;
		else
			$langDirPath = WCP_LANG_DIR. WCP_WPLANG. DS;
