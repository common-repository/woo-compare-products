...
class fieldAdapterWcp {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterWcp', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsWcp'] = array();
    ...
    static public function userFieldDestToDB($value) {
        return utilsWcp::jsonEncode($value);
    }
    static public function userFieldDestFromDB($value) {
        return utilsWcp::jsonDecode($value);
    }
    static public function taxDataHtml($field) {
        ...
        $brands = frameWcp::_()->getModule('products')->getBrands();
        $cOptions = array();
        $bOptions = array();
        if(!empty($categories)) {
        ...
        return '<div>'. __('Apply To', WCP_LANG_CODE). '
            <div id="tax_address">
                <b>'. __('Address', WCP_LANG_CODE). '</b><br />
                '. __('Destination', WCP_LANG_CODE). ':'. htmlWcp::checkboxlist('params[dest]', array('optionsWcp' => $listOfDest)). '<br />
                '. __('Country', WCP_LANG_CODE). ':'. htmlWcp::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. __('Categories', WCP_LANG_CODE). '</b><br />
                '. (empty($cOptions) ? __('You have no categories', WCP_LANG_CODE) : htmlWcp::checkboxlist('params[categories][]', array('optionsWcp' => $cOptions))). '<br />
                    <b>'. __('Brands', WCP_LANG_CODE). '</b><br />
                '. (empty($bOptions) ? __('You have no brands', WCP_LANG_CODE) : htmlWcp::checkboxlist('params[brands][]', array('optionsWcp' => $bOptions))). '<br />
            </div>
            <div>'. __('Tax Rate', WCP_LANG_CODE).': '. htmlWcp::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. __('Absolute', WCP_LANG_CODE).': '. htmlWcp::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? __('Not selected', WCP_LANG_CODE) : __($notSelected);
			}
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frameWcp::_()->getModule('optionsWcp')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= __($key).':<br />';
                $output .= htmlWcp::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= __('class', WCP_LANG_CODE).':<br />';
                $output .= htmlWcp::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= __('id', WCP_LANG_CODE).':<br />';
                $output .= htmlWcp::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsWcp']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsWcp'][0] = in_array(0,$field->value['categories'])?__('Deselect All'):__('Select All', WCP_LANG_CODE);
            foreach($categories as $c) {
                $field->htmlParams['optionsWcp'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frameWcp::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frameWcp::_()->getTable('states')
				->leftJoin( frameWcp::_()->getTable('countries'), 'country_id' )
