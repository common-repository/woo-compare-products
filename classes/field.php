...
class fieldWcp {
	 ...
	 */
	public $adapt = array('htmlWcp' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterWcp::_($value, $this->adapt['dbFrom'], fieldAdapterWcp::DB);
		$this->value = $value;
	}
	...
	public function drawHtml($tag, $id) {
		if(method_exists('htmlWcp', $this->html)) {
			$method = $this->html;
			...
			if($this->adapt['html']) {
				fieldAdapterWcp::_($this, $this->adapt['html'], fieldAdapterWcp::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = frameWcp::_()->getModule('optionsWcp')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select', WCP_LANG_CODE));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsWcp'] = $options;
				}
			}
			$htmlContent = htmlWcp::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlWcp::nameToClassId($this->name). '"></div>';
	...
	public function displayValue() {
		...
		switch($this->html) {
			case 'countryList':
				$value = fieldAdapterWcp::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterWcp::displayState($this->value);
				break;
			case 'checkboxlist':
				$options = $this->getHtmlParam('optionsWcp');
				$value = array();
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = frameWcp::_()->getModule('optionsWcp')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsWcp'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlWcp', $method)) {
			$output .= htmlWcp::$method($name, $params);
			$output .= htmlWcp::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = __('Add Checkbox', WCP_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = __('Add Item', WCP_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = __('Add Radio Button', WCP_LANG_CODE);
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterWcp::_($id,'getExtraFieldOptions',fieldAdapterWcp::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.__('Dimensions', WCP_LANG_CODE).':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= __('width', WCP_LANG_CODE).':<br />';
					$output .= htmlWcp::text('params[width]',array('value'=>$width)).'<br />';
					$output .= __('height', WCP_LANG_CODE).':<br />';
					$output .= htmlWcp::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterWcp::_($this, $this->adapt['htmlParams'], fieldAdapterWcp::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.__('Click to set field "id" and "class"', WCP_LANG_CODE).'</a>';
				$output .= '<div class="attributes" style="display:none;">'.__('Attributes', WCP_LANG_CODE).':<br />';
				$output .= fieldAdapterWcp::_($params,'getFieldAttributes',  fieldAdapterWcp::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	function checkVarFromParam($param, $element) {
		return utilsWcp::xmlAttrToStr($param, $element);
	  ...
	  // load xml structure of parameters
	   ...
	   foreach ($config->params->param as $param) {
		 // read the variables
		  ...
		  $helper = $this->checkVarFromParam($param,'helperWcp');
		  $module = $this->checkVarFromParam($param,'moduleWcp');
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!frameWcp::_()->getModule($module))
			return false; 
		// check for xml file with params structure  
	   if(frameWcp::_()->getModule($module)->isExternal())
		   $config_xml = frameWcp::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = WCP_MODULES_DIR.$module.DS.'mod.xml';

	   if (!file_exists($config_xml)) {
		   // if there is no configuration file for this $module
		   return __('There are no configuration options for this module', WCP_LANG_CODE);
	   }
	   $output = '';
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = frameWcp::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsWcp'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
				  ...
				  // output label and html element
					 ...
					 $output .= '</label><br />';
					 $output .= htmlWcp::$method($name,$htmlParams).'<br />';
					 $output .= '</div>';
			   }
			   $i++;
			 $output .= '</div>';
