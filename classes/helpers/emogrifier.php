...
class EmogrifierWcp
{
	...
	/*supsystic*/
	private $_splitCssAndMediaQueryMedia = '';
	/*****/
    /**
     * The constructor.
     *
    ...
    protected function process(DOMDocument $xmlDocument)
    {
		...
		//var_dump($cssParts['css']);
        foreach ($cssRules as $cssRule) {
            // query the body for the xpath selector
			// It didn't worked with "*" symbol
			/*supsystic*/
			if($this->_isSelectorInvalid($cssRule['selector'])) continue;
			$cssXPath = $this->translateCssToXpath($cssRule['selector']);
			if($this->_isSelectorInvalid($cssXPath)) continue;
			/*****/
			//$cssRule['selector'] = str_replace(array('*'), array('\*'), $cssRule['selector']);
			//var_dump($cssRule['selector'], $cssXPath);
			//var_dump($cssRule['selector']);
     ...
     * node.
     *
     * @param stringarray() $styles the new CSS styles taken from the global styles to be applied to this node
     ...
     *                      right: auto.
     *
     * @return stringarray() an array of values for top, right, bottom and left (using these as associative array keys)
     */
     ...
     *
     * @return stringarray()array() an array of string sub-arrays with the keys
    ...
    private function normalizeStyleAttributes(DOMElement $node)
    {
        $normalizedOriginalStyle = preg_replace_callback(
            '/[A-z\\-]+(?=\\:)/S',
            array($this, 'normalizeStyleAttributesReplaceClb'),
	...
	/*supsystic*/
	public function normalizeStyleAttributesReplaceClb($m) {
     ...
     *
     * @param stringarray() $oldStyles
     * @param stringarray() $newStyles
     *
     * @return string
     */
     ...
     * @param string $css
     *
     * @return stringarray()array() numeric array with string sub-arrays with the keys "css" and "query"
     */
    ...
    {
		/*supsystic*/
		if($this->_isSelectorInvalid( $cssSelector )) {
			return false;
		}
		$cssXPath = $this->translateCssToXpath($cssSelector);
		if($this->_isSelectorInvalid( $cssXPath )) {
			return false;
		}
		/*****/
        $nodesMatchingSelector = $xPath->query($cssXPath);

        return $nodesMatchingSelector !== false && $nodesMatchingSelector->length !== 0;
    }
	
	/*supsystic*/
	private function _isSelectorInvalid($check) {
		// We found that code does not compute those selector correct - so we desided just ignore it for now
		//if(strpos($check, '*') !== false) return true;
		//if(strpos($check, '[') !== false && strpos($check, '"') !== false) return true;
		return false;
	}
	/*****/
    ...
    private function splitCssAndMediaQuery($css)
    {
        ...
        $media = '';
		$this->_splitCssAndMediaQueryMedia = '';
        $cssForAllowedMediaTypes = preg_replace_callback(
            '#@media\\s+(?:only\\s)?(?:[\\s{\\(]' . $mediaTypesExpression . ')\\s?[^{]+{.*}\\s*}\\s*#misU',
            array($this, 'splitCssAndMediaQueryReplaceClb'),
            $cssWithoutComments
        );
		$media = $this->_splitCssAndMediaQueryMedia;
        // filter the CSS
        $search = array(
	...
	/*supsystic*/
	public function splitCssAndMediaQueryReplaceClb($matches) {
		$this->_splitCssAndMediaQueryMedia .= $matches[0];
	}
	/*****/
    ...
    private function translateCssToXpath($cssSelector)
    {
        ...
        $lowercasePaddedSelector = preg_replace_callback(
            '/\\s+\\w+\\s+/',
            array($this, 'translateCssToXpathReplaceClb'),
            $paddedSelector
        );
	...
	/*supsystic*/
	public function translateCssToXpathReplaceClb($matches) {
		return strtolower($matches[0]);
	}
	/*****/

    /**
     * @param stringarray() $match
     *
     * @return string
     */
