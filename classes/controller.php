...
abstract class controllerWcp {
	...
	protected function _createModel($name = '') {
		if(empty($name)) $name = $this->getCode();
		$parentModule = frameWcp::_()->getModule( $this->getCode() );
		$className = '';
		if(importWcp($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNameWcp($name. 'Model');
		}
	...
	protected function _createView($name = '') {
		...
		if(importWcp($parentModule->getModDir(). 'views'. DS. $name. '.php')) {
			$className = toeGetClassNameWcp($name. 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(WCP_ADMIN),
				'remove' => array(WCP_ADMIN),
				'restore' => WCP_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - WCP_METHODS or WCP_USERLEVELS
	 */
	...
	public function getModule() {
		return frameWcp::_()->getModule( $this->getCode() );
	}
	...
	public function getListForTbl() {
		...
		$page = (int) reqWcp::getVar('page');
		$rowsLimit = (int) reqWcp::getVar('rows');
		$orderBy = reqWcp::getVar('sidx');
		$sortOrder = reqWcp::getVar('sord');

		// Our custom search
		$search = reqWcp::getVar('search');
		...
		// jqGrid search
		$isSearch = reqWcp::getVar('_search');
		if($isSearch) {
			$searchField = trim(reqWcp::getVar('searchField'));
			$searchString = trim(reqWcp::getVar('searchString'));
		...
		$model = $this->_prepareModelBeforeListSelect($model, $search);
		...
		$res = dispatcherWcp::applyFilters($this->getCode(). '_getListForTblResults', $res);
		$res->ajaxExec();
	}
	public function removeGroup() {
		$res = new responseWcp();
		if($this->getModel()->removeGroup(reqWcp::getVar('listIds', 'post'))) {
