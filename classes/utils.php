...
class utilsWcp {
	...
	static public function unslashMixed($value) {
		if(empty($value)) return $value;
		if(is_array($value)) {
			foreach($value as $k => $v) {
				$value[ $k ] = self::unslashMixed($v);
			}
			return $value;
		} else
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsWcp::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsWcp::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
	...
	static public function getMonthDaysArray() {
		$res = array();
		$days = range(1, 28);
		foreach($days as $day) {
			$res[ $day ] = $day. 'st';
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameWcp::_()->getModule($m))
                return false;
            if(!is_dir(frameWcp::_()->getModule($m)->getModDir()))
                return false;
        }
        return true;*/
        foreach(frameWcp::_()->getModules() as $m) {
	...
	static public function isPluginsPage() {
		return (basename(reqWcp::getVar('SCRIPT_NAME', 'server')) === 'plugins.php');
	}
	...
	/**
	 * Parse worwcpess post/page/custom post type content for images and return it's IDs if there are images
	...
	static public function getReferalUrl() {
		// Simple for now
		return reqWcp::getVar('HTTP_REFERER', 'server');
	}
	...
	static private function _keyBeforeColon($str) {
		list($k, $v) = explode(':', $str);
		return trim($k);
	}
	static private function _valueAfterColon($str) {
		list($k, $v) = explode(':', $str);
		return trim($v);
	}
	static private function _implodeByColon($k, $v) {
		return $k . ':' . $v;
	}
	static private function _notEmpty($v) {
		return !empty($v);
	}
	/**
	 * sets style value for all selected elements in $content
	 * @param string $content html content to change
	 * @param string $xPathQuery an xpath query returning elements to add style
	 * @param array $styles array of styles to set: [style_key => style_value, ...]
	 * @return string
	 */
	static public function setElementsStyle($content, $xPathQuery, $styles) {
		$doc = new DOMDocument();
		...
		foreach($elements as $element) {
			...
			$currentStyleEntries = array_filter(explode(';', $element->hasAttribute('style') ? $element->getAttribute('style') : ''), 'utilsWcp::_notEmpty');
			
			$currentStyles = empty($currentStyleEntries) ? array() : array_combine(
				array_map('utilsWcp::_keyBeforeColon', $currentStyleEntries),
				array_map('utilsWcp::_valueAfterColon', $currentStyleEntries)
			);
			$currentStyles = array_merge($currentStyles, $styles);
			$currentStyleEntries = array_map('utilsWcp::_implodeByColon', array_keys($currentStyles), $currentStyles);
			if (is_array($currentStyleEntries)) {
				$currentStyle = implode(';', $currentStyleEntries);
				$element->setAttribute('style', $currentStyle . ';');
			}
		}
		$fullText = $doc->saveHTML();
		preg_match('#<body>(.*?)</body>#isu', $fullText, $matches);
		return $matches[1];
	}
}
