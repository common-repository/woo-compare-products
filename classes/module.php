<?php
abstract class moduleWcp extends baseObjectWcp {
	...
	public function __construct($d/*, $params = array()*/) {
		...
		if(isset($d['ex_plug_dir']) && !empty($d['ex_plug_dir'])) {
			...
			$this->setExternalDir( utilsWcp::getExtModDir($d['ex_plug_dir']) );
			$this->setExternalPath( utilsWcp::getExtModPath($d['ex_plug_dir']) );
		}
	}
	...
	public function getModDir() {
		if(empty($this->_externalDir)) {
			return WCP_MODULES_DIR. $this->getCode(). DS;
		} else {
	...
	public function getModPath() {
		if(empty($this->_externalPath)) {
			return WCP_MODULES_PATH. $this->getCode(). '/';
		} else {
	...
	protected function _createController() {
		...
		if(file_exists($this->getModDir(). 'controller.php')) {
			$className = '';
			if(importWcp($this->getModDir(). 'controller.php')) {
				$className = toeGetClassNameWcp($this->getCode(). 'Controller');
			}
			if(!empty($className)) {
	...
	protected function _createHelper() {
		...
		if (file_exists($this->getModDir().'helper.php')) {
			...
			importClassWcp($helper, $this->getModDir(). 'helper.php');
	...
	private function _makeAssetHandle( $for, $handleKey = '' ) {
		$handle = array($for, $this->getCode());
		if(!empty($handleKey)) {
			$handle[] = $handleKey;
		}
		return implode('.', $handle);
	}
	public function addAdminScript( $handleKey = '', $path = '' ) {
		$handle = $this->_makeAssetHandle( 'admin', $handleKey );
		if(empty($path)) {
			$path = $this->getModPath(). 'js/'. $handle. '.js';
		}
		frameWcp::_()->addScript( $handle, $path );
	}
	public function addAdminJSVar( $name, $val, $handleKey = '' ) {
		$handle = $this->_makeAssetHandle( 'admin', $handleKey );
		frameWcp::_()->addJSVar( $handle, $name, $val);
	}
	public function addAdminStyle( $handleKey = '', $path = '' ) {
		$handle = $this->_makeAssetHandle( 'admin', $handleKey );
		if(empty($path)) {
			$path = $this->getModPath(). 'css/'. $handle. '.css';
		}
		frameWcp::_()->addStyle( $handle, $path );
	}
}
