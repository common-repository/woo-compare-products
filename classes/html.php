...
class htmlWcp {
    ...
    static public function span($name, $params= array('attrs' => '', 'value' => '')){
        $output = '<span class="'. self::nameToClassId($name). '">'.$params['value'].'</span>';
        return $output;
    }
	...
	static private function _generateCheckList($name, $params, $htmlClb, $checkedClb) {
		...
		if($params['options']) {
			...
			foreach($params['options'] as $k => $val) {
				...
				$id = 'wcpCheckRandId_'. mt_rand(1, 99999);
				$attrs .= ' id="'. $id. '"';
			...
			if($params['display'] == 'col') {
				$out = '<table class="wcpCheckTbl">'. $out. '</table>';
			}
        }
		return $out;
	}
	static public function datetimepicker($name, $params = array('attrs' => '', 'value' => '')) {
        ...
        return self::input($name, array(
                ...
                jQuery(document).ready(function(){
                    jQuery("#'. $id. '").datetimepicker('. json_encode($params). ');
                });
        </script>';
    }
    ...
    static public function img($src, $usePlugPath = 1, $params = array('width' => '', 'height' => '', 'attrs' => '')) {
        if($usePlugPath) $src = WCP_IMG_PATH. $src;
        return '<img src="'.$src.'" '
    ...
    static public function ajaxfile($name, $params = array('url' => '', 'value' => '', 'fid' => '', 'buttonName' => '')) {
		frameWcp::_()->addScript('ajaxupload', WCP_JS_PATH. 'ajaxupload.js');
        $out = '';
        if(strpos($params['url'], 'pl='. WCP_CODE) === false)
			$params['url'] = uriWcp::_(array('baseUrl' => $params['url'], 'pl' => WCP_CODE));
        $out .= self::button(array('value' => __( empty($params['buttonName']) ? 'Upload' :  $params['buttonName'] ), 'attrs' => 'id="toeUploadbut_'.$name.'" class="button"'));
    ...
    static public function statesInput($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true, 'id' => '', 'selectHtml' => '')) {
        ...
        $states = fieldAdapterWcp::getStates($params['notSelected']);
        ...
        if(empty($params['doNotAddJs'])) {
            $res .= '<script type="text/javascript">
                // <!--
                if(!toeStates.length)
                    toeStates = '. utilsWcp::jsonEncode($states). ';
    ...
    static public function countryList($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true)) {
        $params['notSelected'] = isset($params['notSelected']) ? $params['notSelected'] : true;
        $params['options'] = fieldAdapterWcp::getCountries($params['notSelected']);
		if(!isset($params['attrs'])) 
			$params['attrs'] = '';
    ...
    static public function textFieldsDynamicTable($name, $params = array('value' => '', 'attrs' => '', 'options' => array())) {
        ...
        if(!empty($params['options'])) {
            ...
            foreach($params['options'] as $key => $p) {
                $pattern[$key] = htmlWcp::text($name. '[]['. $key. ']');
            }
            ...
            elseif(!is_array($params['value'])) {
                $params['value'] = utilsWcp::jsonDecode($params['value']);
            ...
            do {
                ...
                foreach($params['options'] as $key => $p) {
                    ...
                    $res .= __($p['label']). htmlWcp::text($name. '['. $i. ']['. $key. ']', $paramsForText);
                }
                $res .= $remove. '</div>';
                $i++;
    ...
    static protected function _loadCategoriesOptions() {
        if(empty(self::$categoriesOptions)) {
            $categories = frameWcp::_()->getModule('products')->getCategories();
            if(!empty($categories)) {
    ...
    static protected function _loadProductsOptions() {
        if(empty(self::$productsOptions)) {
            $products = frameWcp::_()->getModule('products')->getModel()->get(array('getFields' => 'post.ID, post.post_title'));
            if(!empty($products)) {
	...
	static public function fontsList($name, $params = array('value' => '')) {
		...
		if(empty($options)) {	// Fill them only one time per loading
			$fontsList = isset($params['standard']) ? utilsWcp::getStandardFontsList() : array_merge(utilsWcp::getFontsList(), utilsWcp::getStandardFontsList());
		...
		if(isset($params['default'])) {
			$params['options'] = array(WCP_DEFAULT => $params['default']);
		}
	...
	static public function slideInput($name, $params = array('attrs' => '', 'checked' => false, 'id' => '')) {
		...
		return '<a class="toeSlideShellWcp" href="#"'. $params['attrs']. '>
			<span class="toeSlideButtWcp"></span>
			<span class="toeSlideOnWcp">'. __('ON'). '</span>
			<span class="toeSlideOffWcp">'. __('OFF'). '</span>
	...
	static public function galleryBtn($name, $params = array()) {
		...
		$buttonParams = $params;
		$buttonParams['value'] = isset($params['btnVal']) ? $params['btnVal'] : sprintf(__('Select %s', WCP_LANG_CODE), strFirstUp($galleryType));
	...
	/*static public function radialProgress($progress, $attrs = array()) {
		frameWcp::_()->addStyle('radial-progress', WCP_CSS_PATH. 'radial-progress.css');
	...
	static public function recaptcha($name, $params = array()) {
		frameWcp::_()->addScript('google.recaptcha', 'https://www.google.com/recaptcha/api.js?render=explicit&onload=wcpInitCaptcha');
		frameWcp::_()->addScript('google.recaptcha.frontend', WCP_JS_PATH. 'google.recaptcha.js');
	...
	static public function draggableCheckboxList($name, $params) {
		$itemHtm = '<div>Select the fields to show in the comparison table and order them by drag&drop (are included also the woocommerce attributes)</div>';
		$itemHtm .= '<div class="wcpCheckboxDrListContainter">';

		if(!empty($params['value']) && is_array($params['value'])) {
			$valueArr = array_flip($params['value']);
		} else {
			$valueArr = array();
		}
		if(!empty($params['options']) && is_array($params['options']) && count($params['options'])) {

			foreach($params['options'] as $itemKey => $itemValue) {
				$itemHtm .= '<div class="wcpCheckBoxListItem">'
					. '<div class="wcpCblItemLabel">'
					. '<i class="fa fa-arrows-v wcpOnePpHandle"></i>'
					. '<input class="wcpCheckboxCblItem" type="checkbox" name="' . $name . '[]" value="' . $itemKey . '"'
					. (isset($valueArr[$itemKey]) ? ' checked="checked" ' : '')
					. '/>'
					. '<span class="wcpSpanCblItem">' . $itemValue . '</span>'
					. '</div>'
					. '</div>';
			}
		}
