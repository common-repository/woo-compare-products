...
class frameWcp {
        ...
        if($activeModules) {
            foreach($activeModules as $m) {
                ...
                if(!empty($m['ex_plug_dir'])) {
                    $moduleLocationDir = utilsWcp::getExtModDir( $m['ex_plug_dir'] );
                }
                if(is_dir($moduleLocationDir. $code)) {
                    $this->_allModules[$m['code']] = 1;
                    if((bool)$m['active']) {
                        importClassWcp($code. strFirstUp(WCP_CODE), $moduleLocationDir. $code. DS. 'mod.php');
                        $moduleClass = toeGetClassNameWcp($code);
    ...
    public function init() {
        ...
        register_activation_hook(  WCP_DIR. DS. WCP_MAIN_FILE, array('utilsWcp', 'activatePlugin')  ); //See classes/install.php file
        register_uninstall_hook(WCP_DIR. DS. WCP_MAIN_FILE, array('utilsWcp', 'deletePlugin'));
		register_deactivation_hook(WCP_DIR. DS. WCP_MAIN_FILE, array( 'utilsWcp', 'deactivatePlugin' ) );
    ...
    public function checkPermissions($code, $action) {
        if($this->havePermissions($code, $action))
            ...
            exit(_e('You have no permissions to view this page', WCP_LANG_CODE));
        }
    }
    /**
    ...
    public function havePermissions($code, $action) {
        ...
        if($mod) {
            ...
            if(!empty($permissions)) {  // Special permissions
                if(isset($permissions[WCP_METHODS])
                    && !empty($permissions[WCP_METHODS])
                ) {
                    foreach($permissions[WCP_METHODS] as $method => $permissions) {   // Make case-insensitive
                        $permissions[WCP_METHODS][strtolower($method)] = $permissions;
                    }
                    if(array_key_exists($action, $permissions[WCP_METHODS])) {        // Permission for this method exists
                        $currentUserPosition = frameWcp::_()->getModule('user')->getCurrentUserPosition();
                        if((is_array($permissions[ WCP_METHODS ][ $action ]) && !in_array($currentUserPosition, $permissions[ WCP_METHODS ][ $action ]))
                            || (!is_array($permissions[ WCP_METHODS ][ $action ]) && $permissions[WCP_METHODS][$action] != $currentUserPosition)
                        ) {
                            $res = false;
                        }
                    }
                }
                if(isset($permissions[WCP_USERLEVELS])
                    && !empty($permissions[WCP_USERLEVELS])
                ) {
                    $currentUserPosition = frameWcp::_()->getModule('user')->getCurrentUserPosition();
					// For multi-sites network admin role is undefined, let's do this here
					if(is_multisite() && is_admin() && is_super_admin()) {
						$currentUserPosition = WCP_ADMIN;
					}
                    foreach($permissions[WCP_USERLEVELS] as $userlevel => $methods) {
                        if(is_array($methods)) {
			...
			if($res) {	// Additional check for nonces
				...
				if(!empty($noncedMethods)) {
					...
					if(in_array($action, $noncedMethods)) {
						$nonce = isset($_REQUEST['_wpnonce']) ? $_REQUEST['_wpnonce'] : reqWcp::getVar('_wpnonce');
    ...
    protected function _extractTables($tablesDir = WCP_TABLES_DIR) {
    ...
    protected function _extractTable($tableName, $tablesDir = WCP_TABLES_DIR) {
        importClassWcp('noClassNameHere', $tablesDir. $tableName. '.php');
        $this->_tables[$tableName] = tableWcp::_($tableName);
    }
    /**
     ...
     * @return object table
     * @example frameWcp::_()->getTable('products')->getAll()
     */
    ...
    public function addScript($handle, $src = '', $deps = array(), $ver = false, $in_footer = false, $vars = array()) {
		if($this->_ignoreJs) return;
		$src = empty($src) ? $src : uriWcp::_($src);
		if(!$ver)
			$ver = WCP_VERSION;
    ...
    /**
     * Add all scripts from _scripts array to worwcpess
     */
    ...
    public function isTplEditor() {
        $tplEditor = reqWcp::getVar('tplEditor');
	...
	public function isAdminPlugOptsPage() {
		$page = reqWcp::getVar('page');
		if(is_admin() && strpos($page, frameWcp::_()->getModule('adminmenu')->getMainSlug()) !== false)
			return true;
		return false;
	}
	...
	public function savePluginActivationErrors() {
		update_option(WCP_CODE. '_plugin_activation_errors',  ob_get_contents());
	}
	public function getActivationErrors() {
		return get_option(WCP_CODE. '_plugin_activation_errors');
	}
