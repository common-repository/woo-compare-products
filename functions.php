 ...
 */
if(!function_exists('dateToTimestampWcp')) {
    function dateToTimestampWcp($date) {
        if(empty($a))
            return false;
        $a = explode(WCP_DATE_DL, $date);
...
if(!function_exists('importWcp')) {
    function importWcp($path) {
        if(file_exists($path)) {
            require($path);
            return true;
        }
...
if(!function_exists('importClassWcp')) {
    function importClassWcp($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), WCP_CODE) !== false) {
					$classFile = preg_replace('/'. WCP_CODE. '/i', '', $classFile);
				}
                $path = WCP_CLASSES_DIR. $classFile. '.php';
			}
            return importWcp($path);
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, WCP_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], WCP_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassNameWcp')) {
    function toeGetClassNameWcp($class) {
        $className = '';
		if(class_exists($class. strFirstUp(WCP_CODE)))
			$className = $class. strFirstUp(WCP_CODE);
		else if(class_exists(WCP_CLASS_PREFIX. $class))
            $className = WCP_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjWcp')) {
    function toeCreateObjWcp($class, $params) {
        $className = toeGetClassNameWcp($class);
        $obj = NULL;
 ...
 */
if(!function_exists('redirectWcp')) {
    function redirectWcp($url) {
        if(headers_sent()) {
 ...
 * @return array
 */
if(!function_exists('prepareParamsWcp')) {
	function prepareParamsWcp(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsWcp::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodeWcp($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCodeWcp')) {
	function prepareFieldCodeWcp($string) {
