...
jQuery.fn.serializeAnythingDeepWcp = function() {
	var toReturn = []
...
jQuery.fn.serializeAnythingWcp = function(addData, returnArray) {
...
function wcp_str_replace(haystack, needle, replacement) {
	var temp = haystack.split(needle); 
	return temp.join(replacement); 
}
/**
...
function toeListableWcp(params) {
	...
	this.draw = function(list, count) {
		this.table.find('tr').not('.wcpExample, .wcpTblHeader').remove();
		var exampleRow = this.table.find('.wcpExample');
		for(var i in list) {
			...
			newRow.removeClass('wcpExample').show();
...
function setCookieWcp(c_name, value, exdays) {
	var exdate = new Date();
...
/**
 * Retrive worwcpess attach ID from image, using img classes
...
function isNumericWcp(n) {
	if(jQuery.isNumeric)
		return jQuery.isNumeric( n );
	return !isNaN(parseFloat(n)) && isFinite(n);
}
function wcpInitCustomCheckRadio( selector ) {
	if(!selector)
		selector = document;
	...
	,	radioClass: 'iradio_minimal'
		...
		// for checkboxHiddenVal type, see class htmlWcp
...
function wcpUpdateCustomSelects( selector ) {
	var $selects = jQuery( selector );
	console.log( $selects );
...
function wcpCallWpMedia(params) {
	params = params || {};
...
function wcpMceMoveToolbar(editor, clientX) {
	/*if(editor._wcpLastOpenedMenu) {
		editor._wcpHidingForMove = true;
		editor._wcpLastOpenedMenu.hide();
		editor._wcpLastOpenedMenu = null;
	}*/
