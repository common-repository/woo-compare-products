    ...
    $.fn.ColorPickerSliders = function(options) {
        ...
        return this.each(function() {
            ...
            function _bindEvents() {
				...
				/*WCPO*/
				// Change finish trigger
				var changeFinishElementsKeys = ['hue', 'saturation', 'lightness', 'opacity', 'red', 'green', 'blue', 'cielightness', 'ciechroma', 'ciehue'];
				for(var i = 0; i < changeFinishElementsKeys.length; i++) {
					elements.sliders[ changeFinishElementsKeys[i] ].parent().on("touchend mouseup", function(ev) {
						_stopUpdate();
					});
				}
				/*****/
			...
			function _stopUpdate() {
				if(settings.onchangefinish && typeof(settings.onchangefinish) === 'function') {
					settings.onchangefinish( container, color );
				}
			}
